...
ggplot(
  ...
  aes(age_group_ablation,
      bootstrap_mean,
      fill = age_group_ablation)
) +
  geom_boxplot() +
  facet_grid(
    inverse_method ~ task
...
eyes_summary <- conn2 %>%
  filter(
    band == "alpha",
    method %in% c("imcoh", "wpli2_debiased")
  ) %>%
  group_by(
    inverse_method,
    method,
...
ggplot(
  alpha_data,
  aes(task, bootstrap_mean, group = subject)
) +
  geom_line(alpha = 0.3) +
  geom_point(size = 2) +
  theme_bw()
...
delta_alpha <- conn2 %>%
  ...
  mutate(
    delta = EyesClosed - EyesOpen
  )

ggplot(delta_alpha, aes(age_mid, delta)) +
  geom_point() +
  geom_smooth(method = "lm") +
  theme_bw() + 
  labs(
...
power_age_main <- power2 %>%
  ...
  summarise(
    ...
    mean_young = mean(
      global_log_power[age_group_main == "young"],
      na.rm = TRUE
    ),
    
    mean_old = mean(
      global_log_power[age_group_main == "old"],
      na.rm = TRUE
    ),
    ...
    p = ifelse(
      n_young > 0 & n_old > 0,
      wilcox.test(
        global_log_power ~ age_group_main,
        exact = FALSE
      )$p.value,
      NA_real_
    ),
  ...
  mutate(
    comparison = "power_old_vs_young_main",
    p_fdr = p.adjust(p, method = "BH")
  ) %>%
  arrange(p_fdr)
...
power_age_ablation <- power2 %>%
  ...
  summarise(
    ...
    mean_young = mean(
      global_log_power[age_group_ablation == "young"],
      na.rm = TRUE
    ),
    
    mean_old = mean(
      global_log_power[age_group_ablation == "old"],
      na.rm = TRUE
    ),
    ...
    p = ifelse(
      n_young > 0 & n_old > 0,
      wilcox.test(
        global_log_power ~ age_group_ablation,
        exact = FALSE
      )$p.value,
      NA_real_
    ),
  ...
  mutate(
    comparison = "power_old_vs_young_ablation",
    p_fdr = p.adjust(p, method = "BH")
  ) %>%
  arrange(p_fdr)
...
power_eyes_tests <- power2 %>%
  ...
  mutate(
    comparison = "power_EyesOpen_vs_EyesClosed",
    p_fdr = p.adjust(p, method = "BH")
  ) %>%
  arrange(p_fdr)
